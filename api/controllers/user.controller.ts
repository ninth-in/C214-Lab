...
async function updateUser(req:Request, res:Response, next: NextFunction): Promise<any>{
  try {
    const {name, produtora, atores, year} = req.body as UserDocument;
    const savedUser: UserDocument = await userService.updateUser(req?.params?.id, {name, produtora, atores, year} as UserDocument)
    return res.status(201).json(savedUser);
    } catch (err: any) {
      next(err);
    }
